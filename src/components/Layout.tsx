      ...
      <header className={`bg-white shadow-sm py-4 sticky top-0 z-10 transition-all duration-300 ${isScrolled ? 'shadow-md' : 'shadow-sm'}`}>
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between">
            {backUrl ? <div className="flex items-center">
                <Link to={backUrl} className="text-gray-600 hover:text-primary transition-colors duration-300 flex items-center">
                  <ArrowLeft size={20} className="ml-2" />
                  <span>بازگشت</span>
                </Link>
              </div> : <Link to="/" className="text-xl font-bold text-primary hover:text-primary/80 transition-colors duration-300 flex items-center">
            ...
            {showSearch && <div className="w-full max-w-md mx-auto animate-fade-in">
      ...
      <footer className="mt-auto py-6 bg-white border-t">
        <div className="container mx-auto px-4">
          <div className="flex flex-col md:flex-row justify-between items-center text-center md:text-right">
            ...
            <p className="text-gray-500 text-sm mt-2 md:mt-0">ساخته شده با عشق توسط کیان داودی برای فارسی‌زبان های جهان</p>
          </div>
        </div>
      </footer>
    </div>;
